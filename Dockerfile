...
RUN apk update && apk add --no-cache git wget openssh && export PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin
...
RUN chmod +x /bin/entrypoint.sh && rm kustomize*
